...
# Notes:
# • Plotly is imported lazily (inside functions) so the app never dies before UI renders.
# • Many guards for empty/invalid input to avoid silent crashes.
...
# ---------------------------
# Diagnostics (helps catch env issues)
# ---------------------------
...
# Read CSV safely (fallback to python engine if needed)
try:
...
with st.sidebar:
    ...
    if minutes_col:
        try:
            ...
            st.warning("Minutes filter skipped due to invalid values.")
...
with tab1:
    ...
    if len(players) < 2:
        ...
        if chart_type.startswith("Radar") and len(metrics) >= 3:
            # Lazy import Plotly only when we need it
        ...
        if target not in names:
            ...
            if exclude_same_team and team_col:
                try:
                    ...
                    st.warning("Team exclusion skipped due to missing/invalid team data.")
